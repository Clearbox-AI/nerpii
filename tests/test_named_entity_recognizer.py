...
def test_add_address_entity():
    ...
    assert recognizer.deny_list == ['Street', 'Rue', 'Via', 'Square', 'Avenue', 'Place', 'Strada', 'St', 'Lane', 
                                    'Road', 'Boulevard', 'Ln', "Rd", "Highway" "Drive", "Av", "Hwy", "Blvd", "Corso", 
                                    "Piazza", "Calle", "Plaza", "Avenida", "Rambla", "C/"]
    ...
    assert recognizer.deny_list == ['Street', 'Rue', 'Via', 'Square', 'Avenue', 'Place', 'Strada', 'St', 'Lane', 
                                    'Road', 'Boulevard', 'Ln', "Rd", "Highway" "Drive", "Av", "Hwy", "Blvd", "Corso", 
                                    "Piazza", "Calle", "Plaza", "Avenida", "Rambla", "C/", "Alley", "Court"]
...
def test_set_model(instance, nlp_model = 'dslim/bert-base-NER'):
    ...
    instance.set_model()
    assert type(instance.model) is type(pipeline("ner", model = model, tokenizer = tokenizer))
...
def test_assign_location_entity_not_enough_confidence_score(instance):
    instance.dict_global_entities = {'email': ['EMAIL_ADDRESS', 'EMAIL_ADDRESS', 'EMAIL_ADDRESS'], 'city': ['LOCATION', 'LOCATION', 'LOCATION'], 'state': ['GPE', 'GPE'], 'university': None, 'person': ['PERSON', 'PERSON'], 'zipcode': None}
...
#def test_assign_model_entities_list not tested perché il risultato del modello non è sempre uguale a se stesso.


def test_assign_organization_entity(instance):
    instance.model_entities = {'university': ['B-ORG', 'B-ORG', 'I-ORG', 'B-ORG', 'B-ORG', 'I-ORG', 'B-ORG', 'B-ORG', 'I-ORG']}
    instance.assign_organization_entity()
    assert instance.dict_global_entities == {'email': None, 'city': None, 'state': None, 'university': {'entity': 'ORGANIZATION','confidence_score': 0.6666666666666666}, 'person': None, 'zipcode': None}

def test_assign_entities_manually(instance):
    instance.assign_entities_manually()
    assert instance.dict_global_entities == {'email': None, 'city': None, 'state': None, 'university': None, 'person': None, 'zipcode': {'entity': 'ZIPCODE', 'confidence_score': 1.0}}
